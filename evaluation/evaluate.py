...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename=f'evaluation/evaluation_{"hybrid" if args.hybrid else "vector"}.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    for item in tqdm(data, total=len(data), desc="Evaluating"):
        ...
        logger.info(f"{'='*60}\n")
        if args.hybrid:
            results = model.hybrid_search(query, vehicle_patterns, business_patterns, fallback_patterns, top_k=10)
        else:
            results = model.vector_search(query, vehicle_patterns, business_patterns, fallback_patterns, top_k=10)
